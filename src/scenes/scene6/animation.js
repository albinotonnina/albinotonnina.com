...
export default {
  ...
  getScrolledPercentage(data, timing) {
    return data.curTop >= timing.begin
      ? Math.abs(
          ((data.curTop - timing.begin) / timing.duration) * 100
        ).toFixed(3)
      : 0;
  },
  ...
  initClickEvents(site) {
    ...
    document.querySelector("#githubsite").addEventListener("click", () => {
      window.open(
        "http://github.com/albinotonnina/albinotonnina.com",
        "_blank"
      );
    });
  ...
  renderSkills(data) {
    for (let i = 0; i < this.skillShape.points.length; i++) {
      if (
        data.curTop > this.skillShape.pos[i] &&
        data.curTop < this.skillShape.pos[i + 1] &&
        this.skillShape.repeat !== i + 1
      ) {
  ...
  renderSkillWords(data) {
    ...
    const scrolledPercentage = this.getScrolledPercentage(
      data,
      this.sceneTiming
    );
    ...
    if (scrolledPercentage > 2 && scrolledPercentage < 45) {
      const shouldExecute =
        Math.abs(scrolledPercentage - this.lastKeyPercentage) >
        keyFreqPercentage;
