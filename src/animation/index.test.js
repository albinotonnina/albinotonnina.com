...
jest.mock("./transitions", () => ({
  transitions: jest.fn(
    () => new Map([["element1", { 100: { opacity: 0 }, 200: { opacity: 1 } }]])
  ),
  duration: 5000,
}));
...
jest.mock("./tickFunction", () => jest.fn());

// Mock the MP4 import
jest.mock("../images/albinotonnina.com.mp4", () => "mock-video.mp4");
...
beforeEach(() => {
  ...
  const mockPlaceholders = [{ observe: jest.fn() }, { observe: jest.fn() }];
...
describe("Scene Component", () => {
  ...
  describe("font replacement effect", () => {
    it("should replace font families with font weights", async () => {
      ...
      expect(mockTextElements[0].setAttribute).toHaveBeenCalledWith(
        "font-weight",
        "100"
      );
      expect(mockTextElements[0].removeAttribute).toHaveBeenCalledWith(
        "font-family"
      );

      expect(mockTextElements[1].setAttribute).toHaveBeenCalledWith(
        "font-weight",
        "300"
      );
      expect(mockTextElements[1].removeAttribute).toHaveBeenCalledWith(
        "font-family"
      );

      expect(mockTextElements[2].setAttribute).toHaveBeenCalledWith(
        "font-weight",
        "400"
      );
      expect(mockTextElements[2].removeAttribute).toHaveBeenCalledWith(
        "font-family"
      );

      expect(mockTextElements[3].setAttribute).toHaveBeenCalledWith(
        "font-weight",
        "900"
      );
      expect(mockTextElements[3].removeAttribute).toHaveBeenCalledWith(
        "font-family"
      );
    });
  ...
  describe("button event handlers", () => {
    ...
    it("should open LinkedIn URL when contacts button is clicked", () => {
      ...
      expect(window.open).toHaveBeenCalledWith(
        "https://www.linkedin.com/in/albinotonnina/"
      );
    });
